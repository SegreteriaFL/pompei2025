...
function fixInitialHashOffset() {
  ...
  function initHeaderAndNav() {
    ...
    // 1) Logo → index.html (corretto per tutte le lingue)
    (function fixLogo() {
      const logoLink =
        $('.site-logo a', header) ||
        $('#siteLogo', header) ||
      ...
      if (logoLink) {
        // Determina il percorso corretto basato sulla lingua corrente
    ...
    // Aggiungi selettore lingua nel menu mobile (solo se non esiste già e siamo su mobile)
    const isMobile = window.innerWidth <= 768;
    if (isMobile) {
      const existingLangInMenu = list.querySelector('.lang-menu-item');
      if (!existingLangInMenu) {
